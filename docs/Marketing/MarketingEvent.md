...
1. Объемные показатели.
    ...
    - система ищет "Реализации товаров", созданные за выбранное количество недель, в которых номенклатура, характеристика, контрагент и точка доставки (если было уточнение) совпадают с теми, что были указаны в ММ;
    - система ищет "Акционные отгрузки", созданные за выбранное количество недель, в которых номенклатура, характеристика, контрагент и точка доставки совпадают с теми, что были указаны в ММ (Важно: в "Реализации товаров" цена может быть указана на дату отгрузки, либо на дату заказа. Эта дата должна попадать в период документа "Акционных отгрузок");
    - далее из "Реализаций" вычитаются приросты продаж по акциям, указанные в "Акционных отгрузках".
    
    Также при расчете учитываются коэффициенты сезонности. Таким образом рассчитается объем продаж без акции на период проведения акции. Можно заполнить объем по сценарию, нажав на кнопку "Заполнить объем" - "По сценарию".
